...
export default class extends Controller {
  ...
  calculateDays() {
    ...
    if (anniversaryDate && currentDate) {
      // Call backend to calculate days difference
      fetch(`/price_calculator/calculate_days?anniversary_date=${encodeURIComponent(anniversaryDate)}&current_date=${encodeURIComponent(currentDate)}`)
        .then(response => response.json())
        .then(data => {
          if (data.days !== undefined) {
            this.daysTarget.value = data.days
            this.calculatePrice()
          } else {
